...
3. Detect "no live session" early and prompt the user to connect their wallet instead of failing in `ac2_sign`.
...
1. Call **`ac2_sign`** with the bytes to sign, a human-readable `description`, and the right `sig_hint`.
