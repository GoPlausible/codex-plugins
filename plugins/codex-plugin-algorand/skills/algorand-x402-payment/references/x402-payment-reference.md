...
The algorand-mcp x402 tools cover the common case (paying for an HTTP resource from Claude). You might still need the raw protocol when:
...
3. **Paying from outside Claude Code**: e.g., a CI job that fetches a paid API. Use the SDK directly.
...
For day-to-day paid-resource access from Claude, always prefer `make_http_request_with_x402`.
